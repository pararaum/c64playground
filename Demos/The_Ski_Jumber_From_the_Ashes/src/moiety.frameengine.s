	...
	.export	frameengine_init
	.export	frameengine_run
	.export	frameengine_frameno
...
.proc	frameengine_init
	...
	sta	frameengine_frameno
	sta	frameengine_frameno+1
convertloop:
	ldy	#0		; Get waiting time LO
	lda	(jobqueueptr),y
	pha			; Delta LO on stack, see below
	clc
	adc	frameengine_frameno		; Add to frameengine_frameno.
	sta	frameengine_frameno
	sta	(jobqueueptr),y	; Now this is an absolute value.
	iny
	lda	(jobqueueptr),y
	pha			; Delta HI on stack, see below.
	adc	frameengine_frameno+1
	sta	frameengine_frameno+1
	sta	(jobqueueptr),y	; Now this is an absolute value.
	inc16	jobqueueptr,.sizeof(Framejob)
	pla			; Delta HI.
	tax
	pla			; Delta LO.
	cmp	#$FF
...
.proc	check_if_frames_are_right
	ldy	#0		; Frame LO
	lda	(jobqueueptr),y
	cmp	frameengine_frameno
	bne	out
	ldy	#1		; Frame HI
	lda	(jobqueueptr),y
	cmp	frameengine_frameno+1
	bne	out
	ldy	#2		; Queue #
	lda	(jobqueueptr),y
...
.proc	frameengine_run
	...
	inc16	frameengine_frameno		; Increment frame number now, as long runners may interfere...
